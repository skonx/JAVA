...
public class TreeTableViewSample extends Application {
	...
	public void start(Stage stage) {
		...
		employees.stream()/*.sorted(Comparator.comparing(Employee::getName))*/.forEach((employee) -> {
		...
		/*
		 * Possible to use the getName() method of Employee class
		 * or nameProperty() sending a SimpleStringProperty instead...
		 */
		empColumn.setCellValueFactory(
				(TreeTableColumn.CellDataFeatures<Employee, String> param) -> 
				/*new ReadOnlyStringWrapper(param.getValue().getValue().getName()*/param.getValue().getValue().nameProperty()
				);
		...
		/*
		 * Sort the tree table view using the email column
		 * Do this after adding all the column otherwise nothing will be sorted
		 */
		empColumn.setSortType(TreeTableColumn.SortType.ASCENDING);
		treeTableView.setSortMode(TreeSortMode.ALL_DESCENDANTS);
		treeTableView.getSortOrder().add(empColumn);
